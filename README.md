...
**Fast loop (macOS):** add the four `ios/*.swift` files to a macOS unit-test target named
`RuntimeSpike`, add `connector/podcast-rss.js` to the target's resources, wire a `URLProtocol`
stub returning the `validation/fixtures.mjs` bodies, and run `ConnectorRuntimeTests`. This gives
fast feedback on the JS bridging (items 2 and 5 above) without a device.

**Real loop (iOS device):** same files in an iOS app + unit-test target. Run on hardware to
validate items 1, 3, and 4 — the ones a simulator and a mock both lie about.
